...
import type { Team } from "@/pages/TeamsPage";
...
function InviteModal({
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      let users;
      if (type === "Workspace")
        users = await WorkspaceServices.getInviteUsers(
          activeWorkspace!.id,
          query
        );
      else
  ...
  const handleInviteUsers = async () => {
    ...
    if (type === "Workspace") {
      ...
      await WorkspaceServices.inviteMembers(
        activeWorkspace!.id,
        newMemberIds,
        inviteRole
      );
      ...
      await TeamServices.createMember(
        activeWorkspace!.id,
        selectedTeam!.id,
        usersToInvite[0],
        inviteRole
    ...
    <Dialog open={isInviteModalOpen} onOpenChange={setIsInviteModalOpen}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Search and add new members to collaborate in this{" "}
            {type?.toLowerCase()}
          </DialogDescription>
        </DialogHeader>
          ...
          {selectedUsers.length > 0 && (
            <div className="space-y-2">
              ...
              <Select value={inviteRole} onValueChange={setInviteRole}>
                ...
                <SelectContent>
                  <SelectItem
                    value={type === "Workspace" ? "ADMIN" : "TEAM_ADMIN"}
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          Full access to team settings and members
                        </div>
                      </div>
                    </div>
                  </SelectItem>
                  <SelectItem
                    value={type === "Workspace" ? "MEMBER" : "TEAM_MEMBER"}
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          Can edit files and manage team content
                        </div>
                      </div>
                    </div>
                  </SelectItem>
                  <SelectItem
                    value={type === "Workspace" ? "VIEWER" : "--select--"}
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          Can view and comment on team content
                        </div>
                      </div>
                    </div>
                  </SelectItem>
