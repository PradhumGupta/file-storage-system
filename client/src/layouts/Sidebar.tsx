...
function Sidebar({ activeTab = 'All files' }: {activeTab: string}) {
  ...
  return (
        ...
        <aside className="w-[300px] h-screen bg-gray-50 flex-shrink-0 p-8 border-r border-gray-200 flex flex-col">
          {/* Top of Sidebar */}
          <div className="mb-10 flex items-center gap-2">
            <ServerIcon size={20} />
            <h1 className="text-2xl font-bold text-gray-800">Dropbox</h1>
          </div>
          {/* Main Navigation */}
          <nav className="space-y-4 mb-8">
            {sidebarNavItems.map((item, index) => (
              workspaceName === 'personal' && (item.name === 'Members' || item.name === 'Teams') ? <></> : 
              <div
                key={index}
                className={`flex items-center gap-4 p-3 rounded-xl transition-colors duration-200 cursor-pointer
                  ${item.name === activeTab ? 'bg-gray-200 text-black' : 'text-gray-600 hover:bg-gray-100'}`}
                onClick={() => {
          ...
          <div className="flex flex-col flex-grow">
            ...
            <div className="space-y-2 overflow-y-auto">
              {folders.map((folder, index) => (
                <div key={index} className="flex items-center gap-2 p-2 rounded-lg text-gray-700 hover:bg-gray-100 transition-colors duration-200 cursor-pointer">
