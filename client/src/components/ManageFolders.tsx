...
import { Search, FolderCheck, FolderKanbanIcon, FolderIcon } from 'lucide-react';
...
const handleSearch = async (query: string) => {
  ...
  useEffect(() => {
    const fetchFolders = async () => {
      setLoading(true);
      try {
        const folders = await FileServices.getPublicFolders(activeWorkspace!.id);
  ...
  const toggleFolderSelection = (folder: Folder) => {
    setSelectedFolders(prev => 
      prev.some(u => u.id === folder.id) ?
        prev.filter(u => u.id === folder.id) : [...prev, folder]
    );
  };
  ...
  const assignFoldersToTeam = async () => {
    ...
    try {
      ...
      await TeamServices.assignFolder(activeWorkspace?.id, selectedTeam, folderIds)
    } catch (error) {
    ...
    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-sm:max-w-xl max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              public folders to add in teams
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            {!loading && searchResults?.length > 0 && (
              <div className="space-y-2">
                <label className="text-sm font-medium">folders found ({searchResults.length})</label>
