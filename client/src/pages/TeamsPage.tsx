...
export default function TeamsPage() {
  ...
  if (teams.length === 0) {
    return (
        ...
        {/* Create Team Modal */}
        <CreateTeamModal isCreateModalOpen={isCreateModalOpen} setIsCreateModalOpen={setIsCreateModalOpen}  />
      </div>
      </div>
      </div>
    );
  ...
  return (
    <div className="flex bg-gray-100 min-h-screen p-4 font-sans">
        ...
        {/* Header */}
          <header className="flex items-center justify-between m-4">
            <div>
              ...
              {selectedTeamData && (
                <p className="text-gray-600 mt-1">{selectedTeamData.description}</p>
              )}
            </div>
          ...
          {!selectedTeamData ? (
            <>
              <div className="mb-6">
                ...
                <p className="text-gray-600">Select a team to view and manage its members</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {teams.map((team) => (
                  <Card 
                    key={team.id} 
                    className={`group hover:shadow-lg transition-all duration-200 cursor-pointer ${selectedTeam?.id === team.id ? 'ring-2 ring-blue-500' : ''}`}
                    onClick={() => setSelectedTeam(team)}
                  >
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between">
                        <div className={`w-12 h-12 ${team.color} rounded-lg flex items-center justify-center mb-2`}>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-600">Manage members and their roles in {selectedTeamData.name}</p>
                  </div>
                </div>
              </div>
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-4 text-xs text-gray-500 mt-1">
                              ...
                              <span>•</span>
                              <span className={`flex items-center space-x-1 ${member.status === 'active' ? 'text-green-600' : 'text-gray-400'}`}>
      ...
      <CreateTeamModal workspaceId={activeWorkspace?.id} isCreateModalOpen={isCreateModalOpen} setIsCreateModalOpen={setIsCreateModalOpen} setTeams={setTeams}  />

      {/* Invite Member Modal */}
      <InviteModal type={"Team"} isInviteModalOpen={isInviteModalOpen} setIsInviteModalOpen={setIsInviteModalOpen} modalName={selectedTeam} />
    </div>
    </div>
  );
}
